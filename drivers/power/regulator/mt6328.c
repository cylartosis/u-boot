...
#define MT6328_LDO_MODE_NORMAL		0
#define MT6328_LDO_MODE_LP		1
#define MT6328_BUCK_MODE_AUTO		0
#define MT6328_BUCK_MODE_FORCE_PWM	1
...
#define MT6328_BUCK(dt_name, vreg, min, max, step, enreg, vosel, vosel_mask,	\
		    voselon, vosel_ctrl, _modeset_reg, _modeset_shift)		\
{										\
	.min_uV = min,								\
	.max_uV = max,								\
	.step_uV = step,							\
...
#define MT6328_LDO_LINEAR(dt_name, _name, min, max, step,		\
	_enable_reg, _status_reg, _vsel_reg, _vsel_mask)		\
{									\
	.dt_node_name = dt_name,					\
	.type = REGULATOR_TYPE_LINEAR,					\
	.step_uV = (step),						\
	.min_uV = (min),						\
	.max_uV = (max),						\
	.vsel_reg = _vsel_reg,						\
...
static struct mt6328_regulator_info mt6328_bucks[] = {
	MT6328_BUCK("buck_vproc", VPROC, 700000, 1493750, 6250, 0x0486, 0x048a,
		    0x7f, 0x048c, 0x0482, 0x0456, 11),
	MT6328_BUCK("buck_vlte", VMODEM, 700000, 1493750, 6250, 0x04d6, 0x04da,
		    0x7f, 0x04dc, 0x04d2, 0x046c, 11),
	MT6328_BUCK("buck_vcore1", VCORE, 700000, 1493750, 6250, 0x0612,
		    0x0616, 0x7f, 0x0618, 0x060e, 0x0442, 11),
	MT6328_BUCK("buck_vsys22", VSYS, 1400000, 2987500, 12500, 0x063a,
		    0x063e, 0x7f, 0x0640, 0x0636, 0x044c, 9),
	MT6328_BUCK("buck_vpa", VPA, 500000, 3650000, 50000, 0x0662, 0x0666,
		    0x3f, 0x0668, 0x065e, 0x0464, 10),
	MT6328_BUCK("ldo_vsram", VSRAM, 700000, 1493750, 6250, 0x0a40, 0x0a88,
		    0x7f, 0x04b4, 0x04aa, 0x0a40, 0),
};

static struct mt6328_regulator_info mt6328_ldo[] = {
	MT6328_LDO("ldo_vcn33_bt", VCN33_BT, ldo_volt_table6, 0x0a14, 1,
		   0x0a54, 0x70, 0x0a10, 0x1),
	MT6328_LDO("ldo_vcn33_wifi", VCN33_WIFI, ldo_volt_table6, 0x0a12, 1,
		   0x0a5a, 0x70, 0x0a10, 0x1),
	MT6328_LDO("ldo_vcama", VCAMA, ldo_volt_table1, 0x0a0c, 1, 0x0a58,
		   0x30, -1, 0),
	MT6328_LDO("ldo_vmc", VMC, ldo_volt_table4, 0x0a20, 1, 0x0a6a, 0x30,
		   0x0a20, 0x1),
	MT6328_LDO("ldo_vmch", VMCH, ldo_volt_table5, 0x0a1c, 1, 0x0a66, 0x30,
		   0x0a1c, 0x1),
	MT6328_LDO("ldo_vemc3v3", VEMC3V3, ldo_volt_table5, 0x0a24, 1, 0x0a64,
		   0x30, 0x0a24, 0x1),
	...
	MT6328_REG_FIXED("ldo_vio18", VIO18, 0x0a36, 14, 1800000, 0x0a36, 0x1),
	MT6328_LDO("ldo_vcamd", VCAMD, ldo_volt_table10, 0x0a3c, 1, 0x0a7a, 0x70, 0x0a3c, 0x1),
	MT6328_REG_FIXED("ldo_vtcxo_0", VTCXO_0, 0x0a00, 1, 2800000, 0x0a00, 0x1),
	MT6328_REG_FIXED("ldo_vtcxo_1", VTCXO_1, 0x0a02, 1, 2800000, 0x0a02, 0x1),
	MT6328_REG_FIXED("ldo_vcn28", VCN28, 0x0a0e, 1, 2800000, 0x0a0e, 0x1),
	MT6328_REG_FIXED("ldo_vaud28", VAUD28, 0x0a04, 1, 2800000, 0x0a04, 0x1),
	MT6328_REG_FIXED("ldo_vaux18", VAUX18, 0x0a06, 1, 2800000, 0x0a06, 0x1),
	MT6328_REG_FIXED("ldo_vio28", VIO28, 0x0a28, 1, 2800000, 0x0a28, 0x1),
	MT6328_REG_FIXED("ldo_vusb33", VUSB, 0x0a1a, 1, 3300000, 0x0a1a, 0x1),
	MT6328_REG_FIXED("ldo_vrtc", VRTC, 0x0a4a, 1, 2800000, -1, 0),
	MT6328_REG_FIXED("ldo_vrf18_0", VRF18_0, 0x0a08, 1, 1825000, 0x0a08, 0x1),
	MT6328_REG_FIXED("ldo_vrf18_1", VRF18_1, 0x0a16, 1, 1825000, 0x0a16, 0x1),
	MT6328_REG_FIXED("ldo_vcn18", VCN18, 0x0a3a, 1, 1800000, 0x0a3a, 0x1),
	MT6328_LDO("ldo_vcamio", VCAMIO, ldo_volt_table9, 0x0a3e, 1,  0x0a84, 0x30, 0x0a3e, 0x1),
	MT6328_REG_FIXED("ldo_tref", TREF, 0x0a44, 1, 1800000, -1, 0),
};
...
*mt6328_find_mode_by_id(int id, struct dm_regulator_mode *modes,
			uint mode_count)
{
